...
class EmployeeWebController extends Controller
{
    ...
    public function edit(Employee $employee): View
    {
        ...
        $companies = Company::where('active', true)->orderBy('name')->get();
        $departments = $this->departmentsForForm();
        $resolvedWorkTolerance = (new WorkToleranceResolver)->resolve($employee, Carbon::now());

        return view('web.employees.edit', compact('employee', 'companies', 'departments', 'resolvedWorkTolerance'));
    }
    ...
    public function update(Request $request, Employee $employee): RedirectResponse
    {
        ...
        $request->validate([
            ...
            'ws_tolerance_mode' => ['nullable', 'string', Rule::in(['', 'daily_dead_band', 'daily_discount', 'clt_event_based', 'clt_event_strict', 'clt_event_progressive_cap'])],
        ...
        DB::transaction(function () use ($request, $employee, $isPending) {
            ...
            if ($request->filled('ws_entry_time')) {
                ...
                $wsTolMode = $request->input('ws_tolerance_mode');
                $wsTolMode = ($wsTolMode !== null && $wsTolMode !== '') ? $wsTolMode : null;
