...
Endpoints:
  ...
  POST /identify — identifica o funcionário entre uma lista de IDs (modo totem)
...
app = FastAPI(title="Ponto Digital — Face Service", version="2.1.0")
...
async def identify(
    ...
    """
    Identifica qual funcionário da lista corresponde ao rosto enviado.
    employee_ids: lista separada por vírgula dos IDs a comparar (ex: "1,2,3,5")
    Retorna o melhor match dentro do threshold.
    """
    _auth(x_face_service_key)

    ids = [eid.strip() for eid in employee_ids.split(",") if eid.strip()]
    if not ids:
        raise HTTPException(status_code=422, detail="Nenhum employee_id fornecido.")
    ...
    for eid in ids:
        stored = store.get(str(eid))
        if stored is None:
            continue
        dist = _cosine_distance(stored, live_embedding)
        if dist < best_distance:
            best_distance = dist
            best_id = eid

    if best_id is None or best_distance > SIMILARITY_THRESHOLD:
        return {
            "match": False,
            "employee_id": None,
            "score": 0.0,
            "distance": round(best_distance, 4) if best_id else 1.0,
            "threshold": SIMILARITY_THRESHOLD,
