...
<head>
    ...
    <style>
        ...
        .nav-item .nav-icon {
            ...
            color: rgba(165,180,252,0.65); transition: color 150ms;
        }
        .nav-item:hover .nav-icon, .nav-item.active .nav-icon { color: #a5b4fc; }
        ...
        .nav-group-trigger:hover { background: rgba(255,255,255,0.06); color: #fff; }
        .nav-group-trigger.has-active { color: #c7d2fe; }
        .nav-group-trigger .nav-icon { width: 0.9rem; height: 0.9rem; flex-shrink: 0; color: rgba(165,180,252,0.6); }
        .nav-group-trigger:hover .nav-icon, .nav-group-trigger.has-active .nav-icon { color: #a5b4fc; }
        .nav-group-chevron {
            width: 0.75rem; height: 0.75rem; margin-left: auto; flex-shrink: 0;
        ...
        .nav-section {
            ...
            text-transform: uppercase; color: rgba(165,180,252,0.4);
...
<aside id="sidebar"
    ...
    @php
        ...
        $inPessoas  = request()->routeIs(['painel.employees.*','painel.departments.*','painel.payslips.*','painel.communications.*','painel.vacation-requests.*']);
    ...
    <nav id="sidebar-nav" class="flex-1 overflow-y-auto px-3 py-2">
        ...
        {{-- ══ GRUPO: PONTO ══ --}}
        <div class="nav-group mt-1" data-group="ponto">
            <button type="button"
                    class="nav-group-trigger {{ $inPonto ? 'has-active open' : '' }}"
                    onclick="navToggle('ponto')">
                <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>
                </svg>
                <span class="flex-1 text-left">Ponto</span>
                @php $totalPontoBadge = $pendingBadge + $pendingAdditions + $pendingHourBank; @endphp
                @if($totalPontoBadge > 0 && !$inPonto)
                    <span class="inline-flex items-center justify-center h-[16px] min-w-[16px] rounded-full bg-rose-500 text-white text-[9px] font-bold px-1">
                        {{ $totalPontoBadge > 99 ? '99+' : $totalPontoBadge }}
                    </span>
                @endif
            ...
            <div class="nav-group-items {{ $inPonto ? 'open' : '' }}" id="nav-group-ponto">
                ...
                <a href="{{ route('painel.edit-requests.index') }}"
                   class="nav-item nav-sub {{ request()->routeIs('painel.edit-requests.*') ? 'active' : '' }}">
                    ...
                    @if($pendingBadge > 0)
                        <span class="inline-flex items-center justify-center h-[16px] min-w-[16px] rounded-full bg-rose-500 text-white text-[9px] font-bold px-1">{{ $pendingBadge > 99 ? '99+' : $pendingBadge }}</span>
                    @endif
                </a>
                <a href="{{ route('painel.addition-requests.index') }}"
                   class="nav-item nav-sub {{ request()->routeIs('painel.addition-requests.*') ? 'active' : '' }}">
                    ...
                    @if($pendingAdditions > 0)
                        <span class="inline-flex items-center justify-center h-[16px] min-w-[16px] rounded-full bg-amber-500 text-white text-[9px] font-bold px-1">{{ $pendingAdditions > 99 ? '99+' : $pendingAdditions }}</span>
                    @endif
                </a>
                <a href="{{ route('painel.hour-bank.index') }}"
                   class="nav-item nav-sub {{ request()->routeIs('painel.hour-bank.*') ? 'active' : '' }}">
                    ...
                    @if($pendingHourBank > 0)
                        <span class="inline-flex items-center justify-center h-[16px] min-w-[16px] rounded-full bg-amber-500 text-white text-[9px] font-bold px-1">{{ $pendingHourBank > 99 ? '99+' : $pendingHourBank }}</span>
                    @endif
                </a>
                @can('view-audit-logs')
        ...
        {{-- ══ GRUPO: PESSOAS & RH ══ --}}
        <div class="nav-group mt-1" data-group="pessoas">
            <button type="button"
                    class="nav-group-trigger {{ $inPessoas ? 'has-active open' : '' }}"
                ...
                <span class="flex-1 text-left">Pessoas & RH</span>
                @if($pendingVacation > 0 && !$inPessoas)
                    <span class="inline-flex items-center justify-center h-[16px] min-w-[16px] rounded-full bg-sky-500 text-white text-[9px] font-bold px-1">{{ $pendingVacation }}</span>
                @endif
                <svg class="nav-group-chevron" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5"/>
                </svg>
                ...
                <a href="{{ route('painel.vacation-requests.index') }}"
                   class="nav-item nav-sub {{ request()->routeIs('painel.vacation-requests.*') ? 'active' : '' }}">
                    ...
                    @if($pendingVacation > 0)
                        <span class="inline-flex items-center justify-center h-[16px] min-w-[16px] rounded-full bg-sky-500 text-white text-[9px] font-bold px-1">{{ $pendingVacation > 99 ? '99+' : $pendingVacation }}</span>
                    @endif
                </a>
            </div>
        </div>
        ...
        {{-- ══ GRUPO: RELATÓRIOS ══ --}}
        <div class="nav-group mt-1" data-group="relat">
            <button type="button"
                    class="nav-group-trigger {{ $inRelat ? 'has-active open' : '' }}"
        ...
        <div class="nav-group mt-1" data-group="config">
            <button type="button"
                    class="nav-group-trigger {{ $inConfig ? 'has-active open' : '' }}"
